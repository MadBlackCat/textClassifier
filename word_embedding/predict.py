...
for label in label_list:
    ...
    train_path = './data_set/' + classification_name + '_train_data.tsv'
    dev_path = './data_set/' + classification_name + '_dev_data.tsv'
    test_path = './data_set/' + classification_name + '_test_data.tsv'

    save_model_path = "./model/" + model_name + classification_name + "_model_"
    ...
    word_att_all = TimeDistributed(
        Attention_Layer(units=att_size, attention_level='word', name="att_word", activation='softmax'))(review_encoder)
    ...
    predic_classes = np.max(hist_pre[0], axis=1)
    from sklearn.metrics import f1_score, precision_score, recall_score
    result.append([label, precision_score(label, true_label, predic_classes), recall_score(true_label, predic_classes), f1_score(true_label, predic_classes)])
    ...
    # 1
    # file = h5py.File(save_model_path+"test.h5", 'w')
    # weight = []
