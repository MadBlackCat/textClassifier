...
class CalDataLabel():
    ...
    def extract_data(self, out_path, all_data=None):
        if all_data is None:
        ...
        pd.DataFrame(train_data, columns=["sentiment","review"]).to_csv(out_path+"_train_data.tsv", sep="\t")
        pd.DataFrame(dev_data, columns=["sentiment","review"]).to_csv(out_path+"_dev_data.tsv", sep="\t")
        pd.DataFrame(test_data, columns=["sentiment","review"]).to_csv(out_path+"_test_data.tsv", sep="\t")
